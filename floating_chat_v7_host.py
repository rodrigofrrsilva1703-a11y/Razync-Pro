...
from assistant_workspace import (
    ...
    _invalidate_session_snapshot, _provider_state, _session_snapshot, _store_turn,
)
...
def _pending_action_card(draft: object) -> dict | None:
    ...
    labels = {
        ...
        "update_transaction": "Alteração preparada", "update_obligation": "Atualização preparada",
...
def _contextual_quick_actions(transactions, invoices, das_rows: list[dict], obligations: list[dict]) -> list[dict[str, str]]:
    ...
    defaults = [
        ...
        {"label": "Buscar", "prompt": "Procure um lançamento ou documento"}, {"label": "Prioridades", "prompt": "Verifique tudo e mostre minhas prioridades"},
    ]
...
def render_isolated_chat_v7(*, user: dict, page: str, navigate) -> None:
    ...
    event = razync_chat(
        ...
        is_loading=False, theme=theme, feedback_enabled=bool(messages), key="razync_chat_v7_instance",
    )
