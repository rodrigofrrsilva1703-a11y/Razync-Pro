...
def test_contextual_and_dashboard_ai_buttons_open_floating_chat():
...
def test_productivity_no_longer_links_to_dedicated_assistant_page():
    source = Path("productivity_workspace.py").read_text(encoding="utf-8")
    assert '"Razync IA"' in source
    assert 'razync_floating_open' in source
    assert '("Assistente Razync"' not in source
